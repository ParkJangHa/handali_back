...
public class ApartmentService {
    ...
    //[아파트에 입주한 모든 한달이 조회]
    public List<HandaliDTO.HandaliInApartmentResponse> getAllHandalisInApartments(String token)
    {// 반환 타입 수정
        return handaliRepository.findAllHandalisInApartments()
                .stream()
                .map(HandaliDTO.HandaliInApartmentResponse::fromEntity) // DTO 변환 방식 수정
